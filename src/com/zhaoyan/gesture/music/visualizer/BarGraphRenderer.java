...
public class BarGraphRenderer extends Renderer {
	...
	public void onRender(Canvas canvas, FFTData data, Rect rect) {
		...
		NinePatchDrawable bg = (NinePatchDrawable) resources
				.getDrawable(R.drawable.bar_graph);
		...
		for (int i = 0; i < 14; i++) {
			...
			// Average with previous bars value(reduce spikes / smoother
			// transitions)
			...
			for (int j = 0; j < numBlocks; j++) {
				...
				Rect nRect = new Rect((int) x1, yEnd - blockHeight,
						(int) (x1 + bar_width), yEnd);
				bg.setBounds(nRect);
				bg.draw(canvas);
			}
		}
