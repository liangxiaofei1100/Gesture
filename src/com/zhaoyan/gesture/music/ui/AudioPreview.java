...
public class AudioPreview extends Activity implements OnPreparedListener,
	...
	private Handler mButtonRefresher;
	...
	public void onCreate(Bundle icicle) {
		...
		Log.d(TAG, "CallState:" + tm.getCallState());
		if (tm.getCallState() != TelephonyManager.CALL_STATE_IDLE) {
			Toast.makeText(this, R.string.not_play_in_call, Toast.LENGTH_LONG)
					.show();
			finish();
			return;
		}
	...
	private void stopPlayback() {
		if (mButtonRefresher != null) {
			mButtonRefresher.removeCallbacksAndMessages(null);
		}
	...
	public void setNames() {
		...
		if (TextUtils.isEmpty(mTextLine2.getText())
				|| MediaStore.UNKNOWN_STRING.equals(mTextLine2.getText())) {
			mTextLine2.setText(R.string.unknown_artist_name);
		}
	}
	...
	private void updatePlayPause() {
		...
		if (b != null) {
			...
			mButtonRefresher.postDelayed(new ButtonRefresher(), 200);
		}
	}
	 ...
	 * super.onCreateOptionsMenu(menu); // TODO: if mMediaId != -1, then the
	 * playing file has an entry in the media // database, and we could open it
	 * in the full music app instead. // Ideally, we would hand off the
	 * currently running mediaplayer // to the music UI, which can probably be
	 * done via a public static menu.add(0, OPEN_IN_MUSIC, 0, "open in music");
	 * return true; }
	 * 
	 * @Override public boolean onPrepareOptionsMenu(Menu menu) { MenuItem item
	 * = menu.findItem(OPEN_IN_MUSIC); if (mMediaId >= 0) {
	 * item.setVisible(true); return true; } item.setVisible(false); return
	 * false; }
	 */
